  ...
  */
  ...
  if(args[0] == "view") {
  	if(!args[1]) {
  		...
  		allwarn == "" ? api.sendMessage("[𝐕𝐒🐥] Nhóm bạn chưa có ai bị cảnh cáo", threadID, messageID) : api.sendMessage("Danh sách những thành viên đã bị cảnh cáo:\n"+allwarn, threadID, messageID);
  	}
  }
  	...
  	var info = await api.getThreadInfo(threadID);
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('[𝐕𝐒🐥] Chỉ qtv nhóm mới có thể sử dụng lệnh unban!', threadID, messageID);
	
  	if(!id) return api.sendMessage("[𝐕𝐒🐥] Cần nhập id người cần xóa khỏi danh sách bị cấm vào nhóm", threadID, messageID);
  	bans.banned;
  	if(!mybox.includes(id)) return api.sendMessage("[𝐕𝐒🐥] Người này chưa bị cấm vào nhóm của bạn", threadID, messageID);
			api.sendMessage(`[𝐕𝐒🐥] Đã xóa thành viên có id ${id} khỏi danh sách bị cấm vào nhóm`, threadID, messageID);
  	...
  	msg == "" ? api.sendMessage("[𝐕𝐒🐥] Nhóm bạn chưa có ai bị cấm vào nhóm", threadID, messageID) : api.sendMessage("[𝐕𝐒🐥] Những thành viên đã bị cấm vào nhóm:\n"+msg, threadID, messageID);
  }
  ...
  var reason = "";
		...
		api.sendMessage({body: `[𝐕𝐒🐥] Đã ban thành viên ${arrayname.join(", ")} vĩnh viễn khỏi nhóm với lý do: ${reason}`, mentions: arraytag}, threadID, messageID);
		fs.writeFileSync(__dirname + `/cache/bans.json`, JSON.stringify(bans, null, 2));
}
